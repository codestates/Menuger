...
import ProgressBar from '../recipe_edit/ProgressBar';
import useKeyPress from '../../hooks/useKeyPress';
import useToast from '../../hooks/toast/useToast';

const FILE_NUM_LIMIT = 1;
const PERCENT_TO_RATE_RATIO = 100;
const FILE_UPLOAD_MESSAGE = '이미지 파일을 업로드 해주세요';
const TOO_MANY_FILES_MESSAGE = '1개의 이미지 파일만 올려주세요';
...
const UploadImageForm = ({ setUploadModal, width, setImage }) => {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      setImage(`${REACT_APP_S3_URL}/raw/${presignedData.data[0].imageKey}`);
